...
calculate_consensus_labels <- function(E, k, ref_labels=NULL, method = NULL) {

  # Use the first clustering as reference if none is provided
...
run_RPClu_experiments <- function(data_all,
  ...
  for (alg in algorithms) {
    print(paste(alg,consensus_method, sep="+"))
    tryCatch({
      ...
      for (con_method in names(consensus_labels)) {
...
get_best_ensemble_internal_metrics  <- function(experiment_ids,
  ...
  # Final evaluation across top combinations
...
run_pipeline <- function(data, dataset_name, k, top_features, n_reps=20) {
  ...
  result <- get_best_ensemble_internal_metrics(experiments$first:experiments$last,
                                                 data[,top_features], nk = k)
  ...
  for (i in 1:n_reps) {
    print(i)
    seed <- 100 + i
    experiments_rep <- run_RPClu_experiments(data_all = data,
                                             top_features = top_features,
  ...
  for (i in 1:n_reps) {
    seed <- 100 + i
    experiment_id <- run_clustering_experiments(data_all = data,
                                                top_features = top_features,
                                                algorithms = best_clustering,
                                                consensus_method = best_consensus,
                                                k = k,
...
result <- get_best_clustering_internal_metrics(1606:1655, data$x[,top_features], nk = 5)
